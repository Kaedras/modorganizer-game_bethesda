...
{
  QString result = parseSteamLocation(steamAPPId(), "Fallout New Vegas");
  // EPIC Game Store
  if (result.isEmpty()) {
    /**
...
QList<ExecutableInfo> GameFalloutNV::executables() const
{
  ...
  game.withSteamAPI().withSteamOverlay();
  launcher.withSteamAPI().withSteamOverlay();
  ...
  QList executables = {game, launcher};
